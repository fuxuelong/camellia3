...
public class StringUtils extends org.apache.commons.lang3.StringUtils {
    ...
    public static String replaceBlank(String str, String seprator) {
        String res = str.replaceAll("(\\s|\t|\r|\n)", seprator);
        return res;
    }

    // 将所有的换行符、水平制表符、回车、空格替换成一个空格，再将得到的字符串中的空格替换成seprator字符
    public static String replaceSqlStr(String str, String seprator) {
        String res = "";
        // res=str.replaceAll("(\\s|\t|\r|\n)", " ");
        Pattern p = Pattern.compile("\\s+");
        Matcher m = p.matcher(str.replaceAll("(\\s|\t|\r|\n)", " "));
        res = m.replaceAll(" ");
        res = res.replace(" ", seprator);
        return res;
    }
}
